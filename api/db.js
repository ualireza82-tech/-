...
const JSON_COLUMNS = new Set([
  ...
  // ✅ جدید: cleared_for — نگاشت userId → برش‌زمانیِ «حذف برای من» در
  // مکالمات DM (رجوع کنید به server.js، بخش DM ENGINE). با افزودن آن به
  // این لیست، هرجا این ستون مستقیماً در جاوااسکریپت خوانده شود (نه فقط
  // داخل خودِ SQL از طریق json_extract/json_set) به‌صورت خودکار به آبجکت
  // JS تبدیل می‌شود — دقیقاً مثل رفتار hidden_for.
  'cleared_for',
]);
...
// ----------------------------------------------------------------------
// ✅✅✅ پچ جدید — ریشه‌ی باگ «جستجو خالی برمی‌گردد» ✅✅✅
//
// convertPlaceholders (پایین‌تر) با یک regex ساده‌ی سراسری روی کل متن SQL
// دنبال الگوی «$عدد» می‌گردد تا آن را به «?» تبدیل کند. این regex هیچ
// درکی از کامنت‌های SQL (که با -- شروع می‌شوند) ندارد. اگر یک کامنت
// توضیحی داخل کوئری به‌طور اتفاقی حاوی متنی شبیه «$3» باشد (مثلاً برای
// توضیح‌دادن مقدار یک پارامتر)، regex آن را هم به‌عنوان یک placeholder
// واقعی می‌شمارد و یک مقدار اضافه به آرایه‌ی مقادیر باند‌شده اضافه می‌کند.
//
// نتیجه: تعداد «?»ای که در متن نهایی وجود دارد (شامل آنچه داخل کامنت
// جایگزین شده) با تعداد «?»ای که واقعاً توسط SQLite/libSQL بعد از حذف
// کامنت پردازش می‌شود، یکی نیست — و libSQL با خطای «mismatched
// parameter count» کوئری را رد می‌کند. در server.js دقیقاً همین اتفاق
// برای کوئری /api/users/search (که هم جستجوی عمومی و هم جستجوی شروع
// پیام خصوصی از آن استفاده می‌کنند) رخ می‌داد، چون یک کامنت توضیحی در
// همان کوئری حاوی «$3» بود.
//
// راه‌حل ریشه‌ای و بدون هیچ تغییر در server.js: پیش از هر پردازش دیگری،
// خطوطی که کامل کامنت هستند (با -- شروع می‌شوند) از متن SQL حذف می‌شوند.
// این کار برای *هر* کوئری در کل پروژه (حال یا آینده) این کلاس از باگ را
// برای همیشه می‌بندد، بدون این‌که کوچک‌ترین تغییری در منطق یا خروجی
// هیچ endpoint دیگری ایجاد کند — کامنت‌ها هیچ اثر اجرایی روی SQL ندارند.
// فقط خطوطی که *کاملاً* کامنت هستند حذف می‌شوند (خط با فاصله‌ی ابتدایی
// اختیاری و سپس --)؛ به رشته‌های داخل کوئری که تصادفاً «--» دارند
// دست‌زده نمی‌شود.
// ----------------------------------------------------------------------
