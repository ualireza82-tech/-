// AJ Sports 2026 - Ultimate Backend v4.0
// Complete Backend System - Fully Compatible with Frontend
...
async function initializeDatabase() {
    try {
        ...
        await client.query(`
            ...
            INSERT INTO users (email, username, display_name, avatar_url, verification, is_admin) 
            VALUES ('Shahriyarjadidi@gmail.com', 'admin', 'مدیر سیستم', 'https://ui-avatars.com/api/?name=Admin&background=0D8ABC&color=fff', 'gold', true)
            ON CONFLICT (email) DO UPDATE SET verification = 'gold', is_admin = true;
        `);
    ...
    socket.on('send_message', async (data) => {
        try {
            ...
            for (const row of roomUsers.rows) {
                const recipientSocketId = userSocketMap.get(row.user_id);
                if (recipientSocketId) {
                    io.to(recipientSocketId).emit('notification_alert', {
                        type: 'ROOM_MESSAGE',
                        message: `پیام جدید در ${roomId}`,
                        username: username
                    });
                }
            }
...
// Reset limits on startup and schedule daily reset
setTimeout(resetDailyLimits, 1000);
setInterval(resetDailyLimits, 24 * 60 * 60 * 1000); // Every 24 hours
...
app.get('/api/users/profile/:username', async (req, res) => {
    try {
        ...
        const { me } = req.query; // Current user's username
        ...
        // Get user's tweets (excluding deleted ones)
        const tweetsQuery = `
            SELECT 
...
app.get('/api/tweets/feed', async (req, res) => {
    try {
        const { username } = req.query; // Current user's username
...
app.post('/api/tweets', async (req, res) => {
    try {
        ...
        // Update parent tweet reply count if this is a reply
        if (parentId) {
            await pool.query(
        ...
        // Create notification for parent tweet owner if this is a reply
        if (parentId) {
...
app.delete('/api/tweets/:id', async (req, res) => {
    try {
        ...
        // If this tweet has replies, also soft delete them
        await pool.query(
