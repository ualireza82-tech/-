 ...
 * ✅ اضافه شدن سیستم بررسی حساب کاربری موجود
 */
...
app.get('/', (req, res) => {
  res.json({ 
    ...
    endpoints: {
      auth: '/api/auth/sync',
      auth_check: '/api/auth/check-account', // ✅ اضافه شد
      users: '/api/users/profile/:username',
      users_tweets: '/api/users/:username/tweets',
      stories: '/api/stories',
      stories_following: '/api/stories/following/:username',
      tweets: '/api/tweets',
...
// ✅ API جدید: بررسی وجود حساب کاربری
...
// Sync User (Login/Register) - ✅ بدون تغییر
...
// ======================================================
// 3. STORY SYSTEM API ROUTES - ✅ بدون تغییر
// ======================================================
...
app.get('/api/stories/following/:username', async (req, res) => {
  try {
    ...
    // دریافت استوری‌های همه کاربران (نه فقط فالوینگ)
    const query = `
      SELECT 
    ...
    // گروه‌بندی استوری‌ها بر اساس کاربر
    const storiesByUser = {};
...
// Get User Stories - GET /api/stories/user/:username
...
// ======================================================
// 4. NOTIFICATIONS SYSTEM - ✅ بدون تغییر
// ======================================================
...
// ======================================================
// 8. LIVE MATCHES & ROOMS - ✅ بدون تغییر
// ======================================================
...
app.use((req, res) => {
  ...
  res.status(404).json({ 
    ...
    available_endpoints: {
      ...
      auth_check: 'POST /api/auth/check-account', // ✅ اضافه شد
...
server.listen(PORT, () => {
  ...
  console.log(`✅ Check account API: http://localhost:${PORT}/api/auth/check-account`); // ✅ اضافه شد
  console.log(`📸 Story API: http://localhost:${PORT}/api/stories/following/:username`);
  console.log(`👁️ Specific user stories API: http://localhost:${PORT}/api/stories/user/:username`);
});
