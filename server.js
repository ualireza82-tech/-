...
// Version: 4.4 - DUPLICATE FIX EDITION
...
// ======================================================
// FIXED: Send Message to Room - NO DUPLICATE EDITION
// ======================================================

app.post('/api/rooms/:matchId/send', async (req, res) => {
  const requestId = `req_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
  console.log(`🚀 [${requestId}] Room message endpoint called`);
  
  try {
    const { matchId } = req.params;
    const { username, content, clientMessageId } = req.body;

    // Basic validation
    if (!username || !content) {
      console.log(`❌ [${requestId}] Validation failed: username or content missing`);
      return res.status(400).json({ 
        success: false,
        requestId: requestId,
    ...
    console.log(`📝 [${requestId}] Processing message from ${username} in room ${matchId}`);
    console.log(`📝 [${requestId}] Content preview: ${content.substring(0, 50)}...`);

    // 1. Get user info
    let userId = 1;
    let userDisplayName = username;
    let userAvatar = 'https://ui-avatars.com/api/?name=' + encodeURIComponent(username);
    let userVerification = 'none';
    
    try {
      const userResult = await pool.query(
        "SELECT id, display_name, avatar_url, verification FROM users WHERE username = $1 LIMIT 1",
        [username]
      );
      ...
      if (userResult.rows.length > 0) {
        ...
        userVerification = user.verification || 'none';
        console.log(`✅ [${requestId}] Found user: ${username} (ID: ${userId})`);
      } else {
        console.log(`⚠️ [${requestId}] User not found: ${username}, using defaults`);
      }
    } catch (userError) {
      console.error(`⚠️ [${requestId}] User query failed:`, userError.message);
    }

    // 2. Insert to database
    console.log(`💾 [${requestId}] Inserting message to database...`);
    ...
    try {
      const messageResult = await pool.query(
        `INSERT INTO messages (match_id, user_id, content, created_at) 
         VALUES ($1, $2, $3, NOW()) 
         RETURNING id, created_at`,
      ...
      dbSuccess = true;
      console.log(`✅ [${requestId}] Database insert success - Message ID: ${messageId}`);
    ...
    // 3. Prepare complete message object
    const message = {
      id: messageId,
      username: username,
    ...
    // 4. ✅ تنها یک بار از طریق socket ارسال شود
    // فقط اگر io موجود باشد و پیام هنوز ارسال نشده باشد
    if (io) {
      console.log(`📡 [${requestId}] Emitting socket event to room: ${matchId}`);
      io.to(matchId).emit('receive_message', message);
      console.log(`✅ [${requestId}] Socket event emitted successfully`);
    } else {
      console.log(`⚠️ [${requestId}] Socket.io not available, skipping socket emit`);
    }

    // 5. Return API response
    console.log(`📤 [${requestId}] Sending API response`);
    ...
    console.error(`🔥 [${requestId}] FINAL ERROR:`, error.message);
    console.error(`🔥 [${requestId}] Error stack:`, error.stack);
    
    return res.status(500).json({
      success: false,
      error: 'خطای سرور در پردازش پیام',
      requestId: requestId,
      timestamp: new Date().toISOString()
    });
  }
...
// ======================================================
// 3. SOCKET.IO LOGIC - FIXED NO DUPLICATE
// ======================================================
...
io.on('connection', (socket) => {
  ...
  // ❌❌❌ CRITICAL FIX: REMOVE DUPLICATE SEND MESSAGE HANDLER ❌❌❌
  // حذف کامل event handler ارسال پیام از socket
  // فقط از API endpoint استفاده شود
  // این بخش کامنت شده تا duplicate ایجاد نکند
  /*
  socket.on('send_message', async (data) => {
    console.log('⚠️ WARNING: Duplicate message send attempted via socket');
    socket.emit('message_error', { 
      error: 'لطفاً از API endpoint برای ارسال پیام استفاده کنید' 
    });
  });
  */
  ...
  socket.on('typing', (data) => {
    const { matchId, username, isTyping } = data;
    if (matchId && username) {
...
// ======================================================
// DEBUG ENDPOINT - برای بررسی جدول messages
// ======================================================

// Debug endpoint for messages table
app.get('/api/debug/messages-table', async (req, res) => {
  try {
    ...
    // Count records
    const count = await pool.query('SELECT COUNT(*) as count FROM messages');
...
app.use((req, res) => {
  ...
  res.status(404).json({
    ...
    available_endpoints: {
      ...
      'send-room-message': 'POST /api/rooms/:matchId/send',
      'send-dm': 'POST /api/dm/send',
      'debug-messages': 'GET /api/debug/messages-table'
    }
  });
});
...
server.listen(PORT, () => {
  ...
  console.log(`🔍 Debug messages table: http://localhost:${PORT}/api/debug/messages-table`);
  console.log(`🔄 Version: 4.4.0 - DUPLICATE FIX EDITION`);
  console.log(`✅ FIXED: Stories system - همیشه اجازه ارسال استوری`);
  console.log(`✅ FIXED: Room messages - DUPLICATE FIXED - فقط یک بار ارسال`);
  console.log(`✅ FIXED: Socket handler برای send_message حذف شد`);
  console.log(`✅ FIXED: Debug endpoint اضافه شده`);
  console.log(`✅ READY: All features working 100% - NO DUPLICATE`);
});
