...
// Version: 4.1 - Complete Fixed Edition
...
app.get('/', (req, res) => {
  res.json({
    ...
    endpoints: {
      ...
      rooms: 'POST /api/rooms/:matchId/send',
      send_dm: 'POST /api/dm/send',
  ...
  try {
    ...
    const query = `
      SELECT s.*, 
             ...
             EXISTS(SELECT 1 FROM story_views sv WHERE sv.story_id = s.id AND sv.viewer_id = 
               (SELECT id FROM users WHERE username = $1 LIMIT 1)) as has_viewed
      FROM stories s
...
app.post('/api/stories', async (req, res) => {
  try {
    ...
    // Check permission
    const userResult = await pool.query(
      "SELECT id, verification, can_post_story FROM users WHERE username = $1",
      [username]
    );
    ...
    `, [user.id, text_content.trim(), text_color || '#ffffff', background_color || '#000000']);
...
// ======================================================
// NEW: Send Direct Message (API Endpoint)
// ======================================================
...
app.post('/api/dm/send', async (req, res) => {
  try {
    ...
    console.log('💬 Send DM:', senderUsername, 'to', receiverUsername);

    // Get user IDs
    const [senderResult, receiverResult] = await Promise.all([
      pool.query("SELECT id, display_name, avatar_url, verification FROM users WHERE username = $1", [senderUsername]),
      pool.query("SELECT id, display_name, avatar_url, verification FROM users WHERE username = $1", [receiverUsername])
    ]);
    ...
    if (io) {
      ...
      // Also notify recipient if they're not in the conversation
...
// ======================================================
// NEW: Send Message to Room (API Endpoint)
// ======================================================
...
app.post('/api/rooms/:matchId/send', async (req, res) => {
  try {
    ...
    console.log('💬 Send to room:', matchId, 'by', username);
...
app.post('/api/admin/remove-verification', async (req, res) => {
  try {
    ...
    // Remove verification  
    const result = await pool.query(  
      `UPDATE users SET verification = NULL, can_post_story = false  
       WHERE username = $1   
...
app.post('/api/admin/grant-story-permission', async (req, res) => {
  try {
    ...
    // Grant story permission
    const result = await pool.query(
      `UPDATE users SET can_post_story = true 
       WHERE username = $1 
       RETURNING id, username, display_name, can_post_story`,
      [targetUsername]
    );
    ...
    await pool.query(
      ...
      [
        targetUser.id,
        adminUser.rows[0].id,
        `مجوز ارسال استوری به شما اعطا شد!`
      ]
    );
    ...
    if (recipientSocketId && io) {
      io.to(recipientSocketId).emit('notification_alert', {
        ...
        message: `مجوز ارسال استوری به شما اعطا شد!`
      });
    }

    res.json({
      success: true,
      message: "مجوز استوری با موفقیت اعطا شد",
      user: targetUser
    });
...
server.listen(PORT, () => {
  ...
  console.log(`🔄 Version: 4.1.0 - Ultimate Fixed Edition`);
  console.log(`✅ Added: POST /api/dm/send - Send direct messages via API`);
  console.log(`✅ Added: POST /api/rooms/:matchId/send - Send room messages via API`);
});
