 ...
 * PRO SERVER v2.0.0
 * 
 * ⚠️ IMPORTANT: تمام APIهای جدید در بخش ۷.۵ اضافه شده‌اند
 * ⚠️ هیچ تغییری در APIهای موجود ایجاد نشده
 * ⚠️ کاملاً backward compatible
 */
...
app.get('/api/health', async (req, res) => {
  try {
    ...
    res.json({
      ...
      server: 'AJ Sports 2026 Backend v2.0.0'
    });
  } catch (error) {
...
// ======================================================
// 7.5 🚀 NEW ISOLATED BLOCK SYSTEM MODULE
// ======================================================
// ✅ کاملاً جدا از سایر بخش‌ها
// ✅ بدون کوچکترین تغییر در APIهای موجود
// ✅ با prefix /api/blocks برای ایزوله بودن کامل
// ======================================================

// ------------------------------------------------------
// [BLOCK-01] بلاک کردن کاربر
// ------------------------------------------------------
app.post('/api/blocks/block', async (req, res) => {
  try {
    ...
    const blockerId = users.rows.find(u => u.username === blockerUsername).id;
    const blockedId = users.rows.find(u => u.username === blockedUsername).id;
    ...
    // حذف follow در صورت وجود - بدون خطا
    await pool.query(
    ...
    res.json({ 
      ...
      data: { blocker: blockerUsername, blocked: blockedUsername }
    });
...
app.post('/api/blocks/unblock', async (req, res) => {
  try {
    ...
    const blockerId = users.rows[0].id;
    const blockedId = users.rows[1].id;
...
// ------------------------------------------------------
// [BLOCK-03] بررسی وضعیت بلاک دو طرفه
// ------------------------------------------------------
app.get('/api/blocks/status', async (req, res) => {
  try {
    ...
    let blockedBy = null;
    if (block1.rows.length > 0) blockedBy = user1;
    else if (block2.rows.length > 0) blockedBy = user2;

    res.json({
      is_blocked: block1.rows.length > 0 || block2.rows.length > 0,
...
// ------------------------------------------------------
// [BLOCK-04] دریافت لیست بلاک‌شده‌های یک کاربر
// ------------------------------------------------------
...
// ------------------------------------------------------
// [BLOCK-05] بررسی بلاک بودن در یک لیست (Batch)
// ------------------------------------------------------
...
// ======================================================
// 7.6 🚀 NEW ISOLATED ADMIN TWEET DELETE MODULE
// ======================================================
// ✅ کاملاً جدا از API حذف توییت اصلی
// ✅ بدون تغییر در منطق مالکیت توییت
// ======================================================
...
// ======================================================
// 13. 404 HANDLER - با لیست کامل endpoints
// ======================================================
...
app.use((req, res) => {
  ...
  res.status(404).json({ 
    ...
    available_endpoints: {
      ...
      // 🚀 NEW - Admin Tweet Delete (Isolated)
...
server.listen(PORT, () => {
  console.log('\n' + '='.repeat(60));
  console.log('🚀 AJ Sports 2026 Backend v2.0.0');
  ...
  console.log('  • Block System    ✅ (5 APIs)');
  console.log('  • Admin Delete    ✅ (1 API)');
  console.log('='.repeat(60) + '\n');
});
