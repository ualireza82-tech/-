 ...
 * 
 * ⚠️ IMPORTANT: 
 * - فقط بخش /api/dm/conversation اصلاح شده
 * - سایر بخش‌ها دقیقاً مثل نسخه v2.6.0 هستند
 * - اضافه شدن مقادیر پیش‌فرض برای فیلدهای جدید
...
app.get('/api/health', async (req, res) => {
  try {
    ...
    res.json({
      ...
      server: 'AJ Sports 2026 Backend v2.6.1'
    });
  } catch (error) {
...
// ======================================================
// 9. ✅ FIXED: شروع مکالمه (ایزوله - فقط این بخش اصلاح شده)
// ======================================================
app.post('/api/dm/conversation', async (req, res) => {
  try {
    ...
    if (conv.rows.length === 0) {
      // ✅ اصلاح: INSERT با تمام فیلدهای مورد نیاز برای جلوگیری از خطای NOT NULL
      conv = await pool.query(`
        INSERT INTO conversations (
          ...
          created_at,
          updated_at
        ) VALUES ($1, $2, NOW(), NULL, NULL, 0, 0, NOW(), NOW()) 
        RETURNING *
      `, [id1, id2]);
    }
    ...
    // ✅ دریافت تعداد پیام‌های نخوانده برای کاربر درخواست‌کننده
    const requesterId = u1.rows[0].id;
    const unreadCount = requesterId === id1 ? 
...
server.listen(PORT, () => {
  console.log('\n' + '='.repeat(60));
  console.log('🚀 AJ Sports 2026 Backend v2.6.1');
  ...
  console.log('\n🔧 FIXED IN v2.6.1:');
  console.log('  • DM Conversation    ✅ (Fixed NULL constraint error)');
  console.log('  • Insert Query       ✅ (Added all required fields)');
