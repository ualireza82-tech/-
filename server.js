...
app.get('/api/stories/following/:username', async (req, res) => {
  try {
    ...
    // Format response - فقط استوری‌های خود کاربر را برگردان
    const userStories = {
      username: username,
      display_name: user.rows[0].display_name || username,
      avatar_url: user.rows[0].avatar_url || 'https://via.placeholder.com/150',
    ...
    // فعلاً فقط استوری‌های کاربر جاری را برگردان
    const response = [userStories];

    console.log(`✅ Found ${storiesRes.rows.length} stories for ${username}`);
    res.json(response);
  } catch (error) {
