 ...
 * 
 * ⚠️ IMPORTANT: 
 * - فقط بخش‌های مشخص شده اصلاح شده‌اند
 ...
 * 
 * ✅ NEW FEATURES:
 * - افزودن عکس به توییت (media_url)
 * - افزودن هدر به پروفایل (header_url)
 */
...
// ======================================================
// 1. CONFIGURATION & SETUP - بدون تغییر
// ======================================================
...
app.use(express.json({ limit: '20mb' })); // افزایش limit برای آپلود عکس
...
app.get('/api/health', async (req, res) => {
  try {
    ...
    res.json({
      ...
      server: 'AJ Sports 2026 Backend v2.2.0'
    });
  } catch (error) {
...
// ======================================================
// 4. AUTH & USER MANAGEMENT - ✅ اصلاح شده برای هدر
// ======================================================
...
app.post('/api/auth/check-account', async (req, res) => {
  try {
    ...
    // ✅ اضافه کردن header_url به SELECT
    ...
    if (result.rows.length > 0) {
      ...
      return res.json({ 
        ...
        user: {
          ...
          header_url: user.header_url, // ✅ اضافه شده
...
app.post('/api/auth/sync', async (req, res) => {
  try {
    ...
    // ✅ اضافه کردن header_url به INSERT
    const query = `
...
// Get Profile - ✅ اصلاح شده برای header_url
...
// Get User's Tweets - ✅ اصلاح شده برای media_url
app.get('/api/users/:username/tweets', async (req, res) => {
  try {
    ...
    // ✅ اضافه کردن media_url به SELECT
    const query = `
      SELECT 
...
// Update Profile - ✅ اصلاح شده برای header_url
app.put('/api/users/update', async (req, res) => {
  try {
    const { username, display_name, bio, avatar_url, header_url } = req.body; // ✅ اضافه شدن header_url
    
    if (!username) {
      return res.status(400).json({ error: "نام کاربری الزامی است" });
    }

    // ✅ اضافه کردن header_url به UPDATE
    const query = `
      UPDATE users 
...
// Search Users - ✅ اصلاح شده برای header_url
...
// ======================================================
// 6. NOTIFICATIONS SYSTEM - بدون تغییر
// ======================================================
...
// ======================================================
// 8. TWEET SYSTEM - ✅ اصلاح شده برای media_url
// ======================================================

// Feed - ✅ اصلاح شده برای media_url
...
// Create Tweet - ✅ اصلاح شده برای media_url
app.post('/api/tweets', async (req, res) => {
  try {
    const { username, content, parentId, media_url } = req.body; // ✅ اضافه شدن media_url
    ...
    // ✅ اضافه کردن media_url به INSERT
    ...
    const newTweet = { 
      ...
      media_url: media_url || null // ✅ اضافه شده
    };
...
// Delete Tweet (Owner Only) - بدون تغییر
...
// Get Bookmarks - ✅ اصلاح شده برای media_url
...
// ======================================================
// 7.5 🚀 BLOCK SYSTEM - ✅ اصلاح شده - تضمین ۱۰۰٪
// ======================================================
// ✅ اصلاح بحرانی: شناسایی دقیق blocker و blocked
// ✅ اصلاح بحرانی: بررسی وضعیت دوطرفه
// ✅ بدون کوچکترین تغییر در APIهای موجود
// ✅ کاملاً ایزوله و مستقل
// ======================================================

// ------------------------------------------------------
// [BLOCK-01] بلاک کردن کاربر - ✅ اصلاح شده
// ------------------------------------------------------
app.post('/api/blocks/block', async (req, res) => {
  try {
    ...
    // ✅ اصلاح بحرانی: دریافت کاربران با دو کوئری مجزا
...
// ------------------------------------------------------
// [BLOCK-02] آنبلاک کردن کاربر - ✅ اصلاح شده
// ------------------------------------------------------
...
// ------------------------------------------------------
// [BLOCK-03] بررسی وضعیت بلاک دوطرفه - ✅ اصلاح کامل
// ------------------------------------------------------
app.get('/api/blocks/status', async (req, res) => {
  try {
    ...
    // اگر یکی از کاربران وجود نداشت، بلاک وجود ندارد
    ...
    res.json({
      ...
      blocked_by: blockedBy,      // کاربری که بلاک کرده
      blocked_user: blockedUser,   // کاربری که بلاک شده
      blocked_by_me: blockedBy === user1,  // آیا user1 بلاک کرده؟
      blocked_me: blockedBy === user2      // آیا user2 بلاک کرده؟
    });
...
// ------------------------------------------------------
// [BLOCK-04] دریافت لیست بلاک‌شده‌های یک کاربر - ✅ اصلاح شده
// ------------------------------------------------------
...
// ------------------------------------------------------
// [BLOCK-05] بررسی بلاک بودن در یک لیست (Batch) - ✅ اصلاح شده
// ------------------------------------------------------
app.post('/api/blocks/batch-check', async (req, res) => {
  try {
    ...
    // ✅ اصلاح: دریافت blocker با کوئری مجزا
    const blockerQuery = await pool.query(
      "SELECT id FROM users WHERE username = $1",
      [blockerUsername]
    );
...
// ======================================================
// 7.6 🚀 ADMIN TWEET DELETE MODULE - بدون تغییر
// ======================================================
...
// ======================================================
// 10. LIVE MATCHES & ROOMS - بدون تغییر
// ======================================================
...
// ======================================================
// 12. GLOBAL ERROR HANDLER - بدون تغییر
// ======================================================
...
// ======================================================
// 13. 404 HANDLER - ✅ اصلاح شده برای نمایش new features
// ======================================================
...
app.use((req, res) => {
  ...
  res.status(404).json({ 
    ...
    available_endpoints: {
      ...
      // Tweets - ✅ NEW: media_url support
      feed: 'GET /api/tweets/feed (includes media_url)',
      create_tweet: 'POST /api/tweets (accepts media_url)',
      like_tweet: 'POST /api/tweets/:id/like',
      delete_tweet: 'DELETE /api/tweets/:id',
      bookmark: 'POST /api/tweets/:id/bookmark',
      bookmarks: 'GET /api/bookmarks/:username (includes media_url)',
...
// ======================================================
// 14. SERVER START - ✅ اصلاح شده برای نمایش v2.2.0
// ======================================================

const PORT = process.env.PORT || 3000;
server.listen(PORT, () => {
  console.log('\n' + '='.repeat(60));
  console.log('🚀 AJ Sports 2026 Backend v2.2.0');
  console.log('='.repeat(60));
  console.log(`📡 Server: http://localhost:${PORT}`);
  console.log(`🔌 WebSocket: ws://localhost:${PORT}`);
  console.log(`✅ Health: http://localhost:${PORT}/api/health`);
  console.log('\n📦 Core Modules:');
  console.log('  • Auth & Users    ✅ (header_url added)');
  console.log('  • Stories         ✅');
  console.log('  • Tweets          ✅ (media_url added)');
  console.log('  • Notifications   ✅');
  console.log('  • DMs             ✅');
  console.log('  • Admin           ✅');
  console.log('\n🚀 Block System - FIXED:');
  console.log('  • ✅ [BLOCK-01] Block user - شناسایی دقیق');
  console.log('  • ✅ [BLOCK-02] Unblock user - شناسایی دقیق');
  console.log('  • ✅ [BLOCK-03] Status check - بررسی دوطرفه کامل');
  console.log('  • ✅ [BLOCK-04] Get blocks - لیست بلاک‌ها');
  console.log('  • ✅ [BLOCK-05] Batch check - بررسی گروهی');
  console.log('\n🔧 Admin Modules:');
  console.log('  • Admin Delete Tweet   ✅');
  console.log('='.repeat(60) + '\n');
});

// ======================================================
// 15. GRACEFUL SHUTDOWN - بدون تغییر
// ======================================================
...
// ============================================================================
// سیستم خبری هوشمند AJ Sports - نسخه نهایی
// بدون ذخیره در دیتابیس - دارای ۷ اکانت تخصصی
// ============================================================================

const Parser = require('rss-parser');
const parser = new Parser({
    headers: {
        'User-Agent': 'AJ-Sports-Bot/2.0',
...
class RSSNewsSystem {
    constructor(io) {
        this.io = io;
        this.bots = new Map(); // اکانت‌های خبری
        this.postedItems = new Set(); // GUIDهای ارسال شده (برای جلوگیری از تکرار)
        this.checkIntervals = new Map();
        this.MAX_CACHE_SIZE = 1000; // حداکثر GUID در حافظه

        // ====================================================================
        // تعریف ۷ اکانت خبری با بیو، لوگو و تیک آبی
        // ====================================================================
        this.NEWS_ACCOUNTS = [
            {
                id: 'epl',
                ...
                avatar: 'https://i.postimg.cc/XYZ/epl-logo.png', // لوگوی لیگ برتر
                bio: '⚽ آخرین اخبار و شایعات لیگ برتر انگلستان | همراه با پوشش لحظه‌ای بازی‌ها',
                verification: 'blue',
                interval: 3, // هر ۳ دقیقه
                keywords: ['منچستریونایتد', 'لیورپول', 'آرسنال', 'چلسی', 'منچسترسیتی', 'تاتنهام',
            ...
            },
            {
                ...
                avatar: 'https://i.postimg.cc/XYZ/ipl-logo.png', // لوگوی لیگ ایران
                bio: '🏆 پوشش کامل مسابقات لیگ برتر خلیج فارس | اخبار پرسپولیس، استقلال، سپاهان و...',
                verification: 'blue',
                interval: 3,
                keywords: ['پرسپولیس', 'استقلال', 'سپاهان', 'تراکتور', 'گل گهر', 'ذوب آهن', 'فولاد',
            ...
            },
            {
                ...
                display_name: 'لالیگا 🇪🇸',
                avatar: 'https://i.postimg.cc/XYZ/laliga-logo.png', // لوگوی لالیگا
                bio: '🔴 اخبار لحظه‌ای لالیگا اسپانیا | رئال مادرید، بارسلونا، اتلتیکو مادرید',
                verification: 'blue',
                interval: 4,
                keywords: ['رئال مادرید', 'بارسلونا', 'اتلتیکو مادرید', 'سویا', 'ویارئال', 'رئال سوسیداد',
            ...
            },
            {
                ...
                avatar: 'https://i.postimg.cc/XYZ/seriea-logo.png', // لوگوی سری آ
                bio: '⚫🔵 اخبار سری آ ایتالیا | اینتر، میلان، یوونتوس، ناپولی و...',
                verification: 'blue',
                interval: 4,
                keywords: ['اینتر', 'میلان', 'یوونتوس', 'ناپولی', 'آتالانتا', 'رم', 'لاتزیو', 'فیورنتینا',
            ...
            },
            {
                ...
                avatar: 'https://i.postimg.cc/XYZ/ucl-logo.png', // لوگوی لیگ قهرمانان
                bio: '⭐ پوشش اختصاصی لیگ قهرمانان اروپا | قرعه‌کشی، نتایج، تحلیل بازی‌ها',
                verification: 'gold', // تیک طلایی برای لیگ قهرمانان
                interval: 5,
                keywords: ['لیگ قهرمانان', 'چمپیونزلیگ', 'Champions League', 'UCL', 'لیگ اروپا', 'Europa League',
                          'رئال مادرید', 'بایرن مونیخ', 'منچسترسیتی', 'لیورپول', 'پاری سن ژرمن',
                          'اینتر', 'میلان', 'بارسلونا', 'آرسنال', 'دورتموند']
            },
            {
                id: 'varzeshi',
                username: 'varzesh_news',
                display_name: 'خبر ورزشی 📰',
                avatar: 'https://i.postimg.cc/XYZ/varzesh3-logo.png', // لوگوی خبر ورزشی
                bio: '📡 جدیدترین اخبار ورزشی ایران و جهان | نقل و انتقالات، مصاحبه‌ها، تحلیل‌ها',
                verification: 'blue',
                interval: 2, // سریع‌ترین
                keywords: [] // همه خبرها رو می‌گیره (عمومی)
            },
            {
                id: 'breaking',
                username: 'breaking_sports',
                display_name: 'خبر ورزشی (فوری) 🚨',
                avatar: 'https://i.postimg.cc/XYZ/breaking-logo.png', // لوگوی مخصوص خبر فوری
                bio: '⚠️ فوری‌ترین اخبار ورزشی | لحظه به لحظه با رویدادهای مهم',
                verification: 'gold', // تیک طلایی برای فوری
                interval: 2,
                keywords: ['فوری', 'BREAKING', 'urgent', 'شوک', 'رسمی', 'official', 'اختصاصی',
                          'exclusive', 'جدید', 'BREAKING', '🚨', 'مهم', 'فوری']
            }
        ];

        // لیست RSS Feedهای شما (همه ۱۵ لینک)
        this.RSS_FEEDS = [
        ...
        // کش برای جلوگیری از ارسال تکراری (فقط GUID)
    ...
    // ========================================================================
    start() {
        console.log('📰 راه‌اندازی سیستم خبری هوشمند با ۷ اکانت تخصصی');
        
        // ثبت اکانت‌ها
        for (const bot of this.NEWS_ACCOUNTS) {
            this.bots.set(bot.id, bot);
            console.log(`✅ ${bot.display_name} (${bot.username}) - ${bot.verification === 'gold' ? '✨ طلایی' : '🔵 آبی'}`);
        }
        
        // شروع چک کردن RSSها (هر ۲ دقیقه یکبار)
        setInterval(() => this.checkAllFeeds(), 2 * 60 * 1000);
        
        // چک اولیه بعد از ۵ ثانیه
    ...
    // ========================================================================
    async checkAllFeeds() {
        console.log('🔍 بررسی RSS‌ها برای یافتن اخبار جدید...');
        
        for (const feedUrl of this.RSS_FEEDS) {
            try {
                await this.parseAndProcessFeed(feedUrl);
            } catch (error) {
                console.error(`❌ خطا در پردازش ${feedUrl}:`, error.message);
            }
        }
    }
    ...
    async parseAndProcessFeed(feedUrl) {
        try {
            ...
            for (const item of latestItems) {
                ...
                // تشخیص اکانت مناسب برای این خبر
                const targetBots = this.findTargetBots(newsData);
                
                if (targetBots.length > 0) {
                    // ارسال به هر اکانت مناسب
                    for (const bot of targetBots) {
                        this.publishNews(bot, newsData);
                    }
                    
                    // ذخیره GUID برای جلوگیری از تکرار
            ...
            console.error(`❌ خطا در ${feedUrl}:`, error.message);
        }
    }

    // ========================================================================
    // استخراج اطلاعات از خبر
    // ========================================================================
    extractNewsData(item, feedUrl) {
        ...
        else if (feedUrl.includes('ghatreh')) source = 'قره';
        else if (feedUrl.includes('shahrekhabar')) source = 'شهرخبر';
        ...
        return {
            ...
            source,
            feedUrl,
            content: (item.title + ' ' + description).toLowerCase()
        };
    }

    // ========================================================================
    // تشخیص اکانت‌های مناسب برای خبر
    // ========================================================================
    findTargetBots(newsData) {
        ...
        targets.push(this.bots.get('varzeshi'));
        
        // اکانت خبر فوری اگر کلمات کلیدی فوری باشه
        const breakingBot = this.bots.get('breaking');
        if (breakingBot.keywords.some(keyword => text.includes(keyword.toLowerCase()))) {
            targets.push(breakingBot);
        }
        
        // بررسی اکانت‌های تخصصی
        for (const [id, bot] of this.bots) {
            if (id === 'varzeshi' || id === 'breaking') continue;
            
            if (bot.keywords.some(keyword => text.includes(keyword.toLowerCase()))) {
                targets.push(bot);
            }
        }
    ...
    // ========================================================================
    // انتشار خبر (بدون ذخیره در دیتابیس)
    // ========================================================================
    publishNews(bot, newsData) {
        // ساخت محتوای توییت با فرمت درخواستی
        const content = `📰 ${newsData.title}\n\n` +
                       `${newsData.description}\n\n` +
                       `🔗 لینک خبر: ${newsData.link}\n\n` +
        ...
        const tweet = {
            ...
            content: content,
            media_url: newsData.imageUrl, // عکس خبر
            created_at: new Date(),
            is_rss: true,
            verification: bot.verification,
            source: newsData.source,
            source_link: newsData.link,
            bio: bot.bio // برای پروفایل
        };
        
        // پخش به همه کاربران آنلاین
        this.io.emit('new_tweet', tweet);
        
        console.log(`📢 ${bot.display_name}: ${newsData.title.substring(0, 50)}...`);
    }
