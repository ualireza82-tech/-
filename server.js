...
// ======================================================
// FIXED: Send Message to Room (مشکل اصلی گروه)
// ======================================================
...
app.post('/api/rooms/:matchId/send', async (req, res) => {
  try {
    ...
    console.log('💬 Send to room request:', { 
      matchId, 
      username, 
    ...
    // Ensure match_rooms table exists
    try {
      await pool.query('SELECT 1 FROM match_rooms LIMIT 1');
    } catch (err) {
      // Create table if it doesn't exist
      await pool.query(`
        CREATE TABLE IF NOT EXISTS match_rooms (
    ...
    try {
      ...
      if (userResult.rows.length > 0) {
        ...
        if (io) {
          io.to(matchId).emit('receive_message', simpleMessage);
        }

        return res.json({
          success: true,
          message: simpleMessage,
          note: "پیام با روش ساده‌تر ارسال شد"
        });
      }
    } catch (fallbackError) {
