 ...
 * 
 * ⚠️ IMPORTANT: 
 * - فقط بخش /api/dm/conversation با کوئری پویا اصلاح شده
 * - سایر بخش‌ها دقیقاً مثل نسخه v2.6.1 هستند
 * - اضافه شدن تشخیص خودکار ستون‌های موجود در دیتابیس
 * - تضمین عدم آسیب به سایر APIها
 * - بدون نیاز به تغییر در فرانت‌اند
 */
...
app.get('/api/health', async (req, res) => {
  try {
    ...
    res.json({
      ...
      server: 'AJ Sports 2026 Backend v2.6.2'
    });
  } catch (error) {
...
// ======================================================
// 8. ✅ NEW: سیستم پیشرفته پیام‌رسان (DM Upgrade)
// ======================================================

// ===== دریافت لیست کانورسیشن‌ها با ترتیب درست =====
app.get('/api/conversations/:username', async (req, res) => {
  try {
    ...
    const conversations = result.rows.map(row => {
      const lastMsg = row.last_message_details ? row.last_message_details[0] : null;
    ...
    await pool.query(`
      ...
      SET last_message_time = NOW(),
          ...
          unread_count_user1 = CASE 
            WHEN user1_id = $3 THEN unread_count_user1 + 1 
            ELSE unread_count_user1 
          END,
          unread_count_user2 = CASE 
            WHEN user2_id = $3 THEN unread_count_user2 + 1 
            ELSE unread_count_user2 
          END
      WHERE id = $4
    `, [content, senderId, recipientId, conversationId]);
...
// ===== دریافت پیام‌های یک کانورسیشن با status =====
app.get('/api/messages/:conversationId', async (req, res) => {
  try {
    ...
    const totalUnread = await pool.query(`
      ...
      AND sender_id != $2
      AND (seen_at IS NULL OR seen_at > COALESCE((
        SELECT last_seen 
        FROM user_sessions 
        WHERE user_id = $2 
  ...
  try {
    ...
    if (messageIds && messageIds.length > 0) {
      await pool.query(`
        ...
        SET status = 'seen',
            seen_at = $1,
            seen_by = array_append(seen_by, $2::text)
        WHERE id = ANY($3::int[])
        AND sender_id = $4
        AND (seen_at IS NULL OR NOT ($2::text = ANY(seen_by)))
      `, [now, userId.toString(), messageIds, otherUserId]);
    }
    ...
    if (lastSeenMessageId) {
      await pool.query(`
        ...
        AND id <= $5
        AND (seen_at IS NULL OR NOT ($2::text = ANY(seen_by)))
      `, [now, userId.toString(), conversationId, otherUserId, lastSeenMessageId]);
    }
...
// ======================================================
// 9. ✅ FIXED: شروع مکالمه با کوئری پویا (رفع مشکل نهایی)
// ======================================================
app.post('/api/dm/conversation', async (req, res) => {
  try {
    ...
    if (conv.rows.length === 0) {
      // ✅ دریافت لیست ستون‌های موجود در جدول
      ...
      // ستون‌های اجباری
      let insertColumns = ['user1_id', 'user2_id'];
      let insertValues = ['$1', '$2'];
      let valueParams = [id1, id2];
      let paramIndex = 3;
      
      // اضافه کردن ستون‌های اختیاری اگر وجود داشته باشند
      const optionalColumns = [
        'last_message_time', 'last_message_content', 'last_message_sender_id',
      ...
      optionalColumns.forEach(col => {
        if (columnNames.includes(col)) {
          ...
          if (columnDefaults[col] === 'NOW()') {
            insertValues.push('NOW()');
          } else if (columnDefaults[col] === 'NULL') {
            insertValues.push('NULL');
          } else {
            insertValues.push(`$${paramIndex}`);
            valueParams.push(parseInt(columnDefaults[col]) || 0);
            paramIndex++;
          }
        }
      });
      ...
      conv = await pool.query(insertQuery, valueParams);
      console.log('✅ New conversation created with dynamic query');
    }
    ...
    if (conversation.unread_count_user1 !== undefined && conversation.unread_count_user2 !== undefined) {
    ...
    console.error("❌ DETAILED ERROR in /api/dm/conversation:", {
...
// ======================================================
// 10. DIRECT MESSAGES (Legacy - برای backward compatibility)
// ======================================================
...
app.post('/api/dm/conversation/:conversationId/mark-read', async (req, res) => {
  try {
    ...
    if (userId === user1_id) {
      await pool.query(
        "UPDATE conversations SET unread_count_user1 = 0 WHERE id = $1",
        [conversationId]
      );
    } else {
      await pool.query(
        "UPDATE conversations SET unread_count_user2 = 0 WHERE id = $1",
        [conversationId]
      );
    }
...
// ======================================================
// 16. CRON JOB FOR AUTO DELETE EXPIRED ACCOUNTS
// ======================================================
...
// ======================================================
// 17. ✅ SOCKET.IO با رویدادهای پیشرفته
// ======================================================
    ...
    try {
      ...
      await pool.query(`
        ...
        SET last_message_time = NOW(),
            ...
            END
        WHERE id = $4
      `, [cleanContent, senderId, recipientId, conversationId]);
...
// ======================================================
// 19. 404 HANDLER با لیست کامل endpointها
// ======================================================
...
server.listen(PORT, () => {
  console.log('\n' + '='.repeat(60));
  console.log('🚀 AJ Sports 2026 Backend v2.6.2');
  ...
  console.log('  • Two Ticks System   ✅ (✓ = sent, ✓✓ = seen)');
  console.log('  • Unread Badges      ✅ (Persistent after refresh)');
  console.log('  • Conversation Order ✅ (By last message)');
  console.log('  • Seen Confirmation  ✅ (Real-time via socket)');
  console.log('  • Upload System      ✅ (Direct file upload)');
  console.log('  • User Sessions      ✅ (IP, Device, Location)');
  console.log('  • Country Detection  ✅ (Auto from IP)');
  console.log('  • Account Deletion   ✅ (OTP + 30 Days Auto Delete)');
  
  console.log('\n🔧 FIXED IN v2.6.2:');
  console.log('  • DM Conversation    ✅ (Dynamic column detection)');
  console.log('  • Database Schema    ✅ (Auto-adapts to existing columns)');
  console.log('  • Error Handling     ✅ (Detailed logging)');
  console.log('  • Zero Impact        ✅ (No changes to other APIs)');
